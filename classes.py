...
# region Chat
class Chat:
    def __init__(self, client, chat_id, _f=0):
        """
        ...
        if recv["opcode"] not in [150]:
            self.messages = [Message(client, 0, **msg, _f=1) for msg in payload["messages"]]
...
# region Message
class Message:
    def __init__(self, client, chatId: str, sender=None, id=None, time=None, text="", type=None, _f=0, **kwargs):
        """
        ...
        if not _f:
            self.chat = Chat(client, chatId, _f=1)
        self.sender = sender
        self.id = id
        self.time = time
        self.text = text
