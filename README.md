...
**MAX → Telegram**
- текст, фото, видео (в т.ч. кружки), файлы, голосовые, стикеры, опросы (снимок: вопрос, варианты, голоса)
...
- реакции MAX → TG: ❤/👍 на исходном посте в TG (от бота) + ответ-цитата `❤ Иван` и строка `на: …` (кусок текста)
...
- реакции на эти сообщения в MAX тоже приходят в TG (на твой исходный пост)
...
- опрос MAX → текст в TG (голосовать из TG в MAX нельзя)
- реакции в TG от имени бота, не от одноклассника; не все эмодзи MAX есть в TG
- связка id сообщений только в памяти: после рестарта бота старые посты реакции не получат
- реакции TG → MAX не ставятся
...
4. Рестарт: дома `starter.py`; на NAS — залей `.env` → контейнер **Стоп → Старт**, проверь `/status`
...
3. Поставь ❤ в MAX на свежий пост моста → в TG реакция бота + ответ `❤ Имя` с цитатой и `на: …`
4. `/status` — онлайн, последняя пересылка, пары `id (название)`
