...
class MaxClient:
    ...
    def connect(self, _f=None):
        ...
        raw = json.loads(self.websocket.recv())
        p = raw.get("payload") or {}
        if p.get("error") or not p.get("profile"):
            err = p.get("localizedMessage") or p.get("message") or p.get("error") or "нет profile"
            if self._on_auth_error:
                try:
                    self._on_auth_error(err)
                except Exception:
                    pass
            raise ValueError(f"MAX auth: {err}")
        ...
        for c in p.get("chats") or []:
            cid = c.get("id")
            if cid is not None:
                self._chat_titles[int(cid)] = c.get("title") or c.get("name") or ""

        if self._on_connect:
            self._on_connect()

    def _call(self, packet: dict, opcode=None, timeout=15):
        seq = packet["seq"]
        q = None
        if getattr(self, "_t", None) and self._t.is_alive():
            q = queue.Queue(maxsize=1)
            self._rpc[seq] = (opcode, q)
        ...
        while True:
            ...
            if recv.get("seq") == seq and (opcode is None or recv.get("opcode") == opcode):
                return recv

    def get_history(self, chat_id: int, from_ts: int | None = None, backward: int = 50) -> list:
        seq = self.seq
        return self._call({
            "ver": 11,
            "cmd": 0,
            ...
            "payload": {
                "chatId": chat_id,
                "from": from_ts or int(time.time() * 1000),
                "forward": 0,
                "backward": backward,
                "getMessages": True
            }
        }, 49).get("payload", {}).get("messages") or []

    def get_chat_titles(self, chat_ids: list[int]) -> dict[int, str]:
        missing = [i for i in chat_ids if i not in self._chat_titles]
        if missing:
            try:
                p = self._call({
                    "ver": 11, "cmd": 0, "seq": self.seq, "opcode": 48,
                    "payload": {"chatIds": missing},
                }, 48).get("payload") or {}
            except Exception as e:
                print("get_chat_titles:", e)
                p = {}
            chats = p.get("chats") or p.get("chatInfos") or []
            if isinstance(chats, dict):
                chats = list(chats.values()) if "id" not in chats else [chats]
            elif p.get("chat"):
                chats = list(chats) + ([p["chat"]] if isinstance(p["chat"], dict) else p["chat"])
            for c in chats:
                ...
                if cid is not None and title:
                    self._chat_titles[int(cid)] = title
            for i in missing:
                self._chat_titles.setdefault(i, "")
        return {i: self._chat_titles.get(i, "") for i in chat_ids}
    ...
    # region _listener()
    def _dispatch(self, recv):
        seq = recv.get("seq")
        pending = self._rpc.get(seq)
        if pending:
            exp, q = pending
            if exp is None or recv.get("opcode") == exp:
                q.put(recv)
                return
    ...
    def _listener(self):
        ...
        while not self._t_stop:
            try:
                self._dispatch(json.loads(self.websocket.recv()))
                while True:
                    try:
                        self._dispatch(json.loads(self.websocket.recv(timeout=0.01)))
                    except TimeoutError:
                        break
    ...
    def _inbox_worker(self):
        while not self._t_stop:
            try:
                recv = self._inbox.get(timeout=0.5)
            except queue.Empty:
                continue
            try:
    ...
    def run(self):
        ...
        threading.Thread(target=self._inbox_worker, name="WebMaxInbox", daemon=True).start()
